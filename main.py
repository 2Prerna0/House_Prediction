...
def main():
    ...
    size = st.number_input('House size', min_value=200, max_value=2000, value = 500)
    
    if st.button('Predict price'):
        predicted_price = model.predict(pd.DataFrame([[size]], columns=['size'])) 
        st.success(f'Estimated price: ${predicted_price.item():,.2f}')
        
        
        fig = px.scatter(df,x='size',y='price',title='size vs House price')
        fig.add_scatter(
                x=[size],
                y=[predicted_price.item()],  # 👈 convert to scalar
                mode='markers',
